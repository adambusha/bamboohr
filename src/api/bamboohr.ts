...
async function enrichTimeOffTypes(entries: WhosOutEntry[], subdomain: string, authString: string): Promise<void> {
  // Get information about time off types from the BambooHR API
  const typeUrl = `https://api.bamboohr.com/api/gateway.php/${subdomain}/v1/meta/time_off/types`;
  console.log(`Fetching time off types from: ${typeUrl}`);
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Error fetching time off types: ${response.status} ${response.statusText}`);
    }
    
    const typeData = await response.json();
    console.log("Time off types:", JSON.stringify(typeData));
    
    // Create a map of request IDs to time off types
    for (const entry of entries) {
      if (!entry.timeOffType || entry.timeOffType === "") {
        // Try to find a better name for the time off type
        if (entry.type === "Time Off") {
          entry.timeOffType = "Paid Time Off";
    ...
    console.error("Error enriching time off types:", error);
  }
}

function processResponseItem(item: any, entries: WhosOutEntry[]): void {
  ...
  if (item.employeeId && item.name && item.start && item.end) {
    entries.push({
      ...
      timeOffType: typeof item.timeOffType === 'object' ? item.timeOffType.name : 
                 item.timeOffType || "Paid Time Off",
